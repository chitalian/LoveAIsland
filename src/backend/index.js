  ...
  let agent = {
    ...
    profileData: {
      ...
      prompts: [
        ...
        [
          'I\'m known for',
          'my knowledge of obscure Michigan facts',
        ],
      ],
    },
    interactionHistory: [],
...
function simulateAgent(selfId) {
  ...
  for (const agentId in agentStates) {
    if (agentId !== selfId && nearby(center, agentStates[agentId].position), MAX_POI_DISTANCE) {
  ...
  for (const agentId in agentStates) {
    if (agentId !== selfId && nearby(center, agentStates[agentId].position), MAX_INTERACT_DISTANCE) {
